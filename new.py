...
def create_table():
    con=ms.connect(host='localhost',user='root', password='root81611',charset='utf8',database='accounts')
    cur=con.cursor()
    st="create table if not exists acc_holder (acc_no char(5),name char(20),addr char(20),opn_bal integer,phno integer(11));"
    cur.execute(st)
                    ...
                    if ac in i:
                        ...
                        while True:
                            ...
                            if bal<1000:
                                ...
                                break
                        cur.execute("update acc_holder set opn_bal=opn_bal- '{}' where acc_no='{}'".format(wtdr,ac))
                        con.commit()
                ...
                if acno in i:
                    ...
                    while True:
                        ...
                        if ch==1:
                            ...
                            cur.execute ("update acc_holder set phone='{}'where acc_no='{}'".format(acno))
                            break
                        else:
...
def disp_acct():
    ...
    if ch=='s':
        con=ms.connect(host="localhost",user="root",password="root",charset="utf8",database="accounts")
        cur=con.cursor()
        cur.execute('select*from acc_holder')
        rec=cur.fetchall()
        ...
        for i in rec:
            ...
            for i in rec:
                ...
                print("RECORD FOUND")
                print(i)
                cur.execute('delete from acc_holder where acc_no="{}".format(nm)')
                ch=int('are you sure to delete(y/n)?')
                if ch=='y':
                    con.commit()
...
def del_acct():
    ...
    if con.is_connected():
        ...
        for i in rec:
            print(i)
        nm=input('enter account to delete=')
        for i in rec:
            if nm in i:
                print('recordfound')
                print(i)
